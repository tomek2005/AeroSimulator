...
public class SystemsPanelWidget : IWidget
{
    ...
    public void Render()
    {
        ...
        Console.WriteLine(
            $" Fuel: {displayFuel,6:F0} KG ({fuelStatus}) | Elec Main: {elec.MainBusVoltage,4:F1}V | Sec: {elec.SecondaryBusVoltage,4:F1}V | De-Ice: {(elec.IsDeIcingActive ? "ON" : "OFF")}");
        ...
        string gearStatus = hydr.GearJammed
            ...
            : hydr.IsGearTransiting
                ...
                : hydr.IsGearExtended
                    ...
                    : "UP";
        Console.WriteLine(
            $" Hydr Press: {hydr.Pressure,6:F0} PSI  | Gear: {gearStatus,-10} | Flaps: {wing.FlapsPosition * 100,3:F0}% | Spoilers: {(wing.SpoilersDeployed ? "UP" : "DOWN")}");
        
        string apStatus = ap.IsOffline ? "OFFLINE" : (ap.IsEngaged ? "ENGAGED" : "STDBY");
        Console.WriteLine(
            $" Autopilot:  {apStatus,-10} | WX: {weather.CurrentCondition,-16} ({weather.SecondsToNextChange,2:F0}s) | Radar: {(weather.IsOffline ? "OFF" : "ON "),-3} | Wind: {_aircraft.FlightData.WindDirectionDeg,3:F0}/{_aircraft.FlightData.WindSpeedKnots,2:F0} kt | Ice: {wing.IceAccumulation * 100,3:F0}% {(wing.IsIceCritical() ? "[CRITICAL!]" : "")}");
